...
public class Autobus {
    ...
    //Idioma ingles
    private final String VEHICLE_REGISTRATION = "vehicle registration";
        ...
        switch(idioma){
            case Viaje.CASTELLANO:
                ...
                for (int i = 1; i <= asientos.size() ; i++) {
                    asientos.get(i).generarHojaViaje(fichero, idioma);    
                }      
                break;
            case Viaje.INGLES:
                fichero.write(VEHICLE_REGISTRATION + matricula + "\n");
                for (int i = 1; i <= asientos.size() ; i++) {
                    asientos.get(i).generarHojaViaje(fichero, idioma); 
                    }
                break;
